...
class LocationsViewController: UITableViewController {
    ...
    lazy var fetchedResultsController: NSFetchedResultsController<Location> = {
        ...
        let fetchedResultsController = NSFetchedResultsController(fetchRequest: fetchRequest, managedObjectContext: self.context, sectionNameKeyPath: "category", cacheName: "Locations")
  ...
  //      label.text = self.tableView(tableView, titleForHeaderInSection: section)
        ...
        let separatorRect = CGRect(
            x: 15, y: tableView.sectionHeaderHeight - 0.5, width: tableView.bounds.size.width - 15, height: 0.5)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "EditLocation"{
            ...
            if let indexPath = tableView.indexPath(for: sender as! UITableViewCell){
                destination.locationToEdit = fetchedResultsController.object(at: indexPath)
            }
